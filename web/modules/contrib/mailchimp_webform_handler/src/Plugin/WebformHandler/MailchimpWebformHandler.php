...
namespace Drupal\mailchimp_webform_handler\Plugin\WebformHandler;
 ...
 *   label = @Translation("Mailchimp Webform Handler"),
...
class MailchimpWebformHandler extends WebformHandlerBase {
  ...
  public function getSummary() {
    return [
      '#markup' => $this->t('No summary available'),
    ];
  }
  ...
  protected function getLists() {
    ...
    if (isset($this->configuration['api_key'])
      && isset($this->configuration['server_prefix'])
      && $this->configuration['api_key'] != ''
      && $this->configuration['server_prefix'] != '') {
      $client = new MailchimpMarketing\ApiClient();
      $client->setConfig([
        'apiKey' => $this->configuration['api_key'],
        'server' => $this->configuration['server_prefix']
